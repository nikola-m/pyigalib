# p17 - Helmholtz eq. on [-1,1]x[-1,1] with u=0 on boundary
...
# RHS vector
k = 9.
f = np.exp(-10.*((y-1.)**2+(x-0.5)**2))

# PDE operator-tensor product Laplacian
L = np.kron(N,D2)+np.kron(D2,N)+k**2*np.kron(N,N)
# condition number
...
ax.plot_surface(xx, yy, uu, rstride=1, cstride=1, cmap=cm.jet, linewidth=0.3, antialiased=True, alpha=0.7)
plt.title('Helmholtz eq., p = %d, n = %d, time = %f [s]' % (p,n+1,elapsed))
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('u')
plt.show()
